...
$template = file_get_contents($CFG->dirroot . '/local/parentschat/template/message_reply.mustache');
...
$mustache = new Mustache_Engine();
    ...
    $data = $DB->get_record_sql("SELECT * FROM {parentschat} WHERE id=$mid AND  pid = ?", array($pid1->user_id));
    ...
    $tableRows = [];
        ...
        $tid1 = $data->tid;
       $msgid=$data->id;
       $reply = $DB->get_record_sql("SELECT * FROM {teacherchat} WHERE msgid=$msgid");
