...
require_once($CFG->dirroot.'/local/new_timetable/new_period_form.php');
...
// Set the url.
$linkurl = new moodle_url('/local/new_timetable/new_period.php');
...
$PAGE->navbar->add('Time table', new moodle_url($CFG->wwwroot.'/local/new_timetable/new_period.php'));

echo $OUTPUT->header();
$mform = new new_period_form();
...
if($mform->is_cancelled()){
    ...
    $id1= $DB->get_record_sql("SELECT id FROM {new_timetable_periods} WHERE t_day='$id'");
    // print_r($id1->days_id);exit();
     $timetabledata->period_id =$id1->id;
     $timetabledata->days_id =$id;
    //  print_r($timetabledata->days_id);exit();
...
// Get the current value of the count field
$current_count = $DB->get_field('new_timetable_periods', 't_periods', array('t_day' => $id));
// print_r($current_count);exit();
// Increment the count by 1
$new_count = $current_count + 1;
// print_r($new_count);exit();
$id2=$DB->get_record_sql("SELECT id FROM {new_timetable_periods} WHERE t_day='$id'");
//print_r($id2->id);exit();
// Update the database with the new count value
$update_data = new stdClass();
$update_data->id = $id2->id;
