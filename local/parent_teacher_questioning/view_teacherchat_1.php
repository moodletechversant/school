...
$template = file_get_contents($CFG->dirroot . '/local/parent_teacher_questioning/template/viewteacherchat_1.mustache');
...
$linkurl = new moodle_url('/local/parent_teacher_questioning/view_teacherchat_1.php');
$css_link = new moodle_url('/local/css/style.css');
$view_message =  new moodle_url('/local/parent_teacher_questioning/reply_teacher.php?id');
$teacherchat_link= new moodle_url('/local/parent_teacher_questioning/parentschat.php');
...
if ($tid->user_id == $userid) {
    $data = $DB->get_records_sql("SELECT * FROM {parents_enquiry} WHERE tid = ?", array($tid->user_id));
...
$output = $mustache->render($template, ['tableRows' => $tableRows, 'css_link' => $css_link, 'view_message' => $view_message ,'teacherchat_link' => $teacherchat_link]);
