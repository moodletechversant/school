...
// Set the url.
$linkurl = new moodle_url('/local/teacherassign/editassign.php');
...
$PAGE->navbar->add('Student', new moodle_url($CFG->wwwroot.'/local/teacherassign/editassign.php'));

echo $OUTPUT->header();
$mform = new edit_assignstudent_form();

    if($mform->is_cancelled()){
        $cancelurl = $CFG->wwwroot.'/local/teacherassign/view_Tassign.php';
        redirect($cancelurl);
    }
        ...
        $stddata->t_class=$formdata->class;
        $stddata->t_division=$formdata->division;
        $stddata->t_subject=$formdata->subject;
        $stddata->user_id=$formdata->teacher;

        $teach_ids = $DB->get_records_sql("SELECT user_id FROM {teacher_assign} WHERE id =$formdata->id");  
        foreach ($teach_ids as $teacherid) {
                $userIds = $teacherid->user_id;
