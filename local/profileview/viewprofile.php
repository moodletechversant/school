...
$sid = $DB->get_record_sql("SELECT * FROM {student} WHERE user_id = ?", array($user));
    ...
    elseif ($sid->user_id) {
      $student = $DB->get_record_sql("SELECT * FROM {student} WHERE user_id=$sid->user_id");
      // print_r($student);exit();
      $sprofile = array(); 
      $scourses1 = array(); 
      
      $studentt = $DB->get_record_sql("SELECT * FROM {student_assign} WHERE user_id=$sid->user_id");
      if(!empty($studentt)){
      $s_division = $studentt->s_division;
      $value=$DB->get_record_sql("SELECT * FROM {division} WHERE id=$s_division");
      $value1=$DB->get_record_sql("SELECT * FROM {teacher} WHERE user_id=$value->div_teacherid");
